...
export const medicalRecordService = {
     ...
     */
    getDoctorPrescriptions: async (doctorName: string): Promise<MedicalTimelineItem[]> => {
        // Filter runtime records where provider matches doctor name
        return RUNTIME_RECORDS.filter(r => r.provider === doctorName && r.type === 'PRESCRIPTION');
    },

    /**
     * Update a record's status or details
     */
    updateRecord: async (id: string, updates: Partial<MedicalTimelineItem>): Promise<boolean> => {
        const recordIndex = RUNTIME_RECORDS.findIndex(r => r.id === id);
        if (recordIndex !== -1) {
            RUNTIME_RECORDS[recordIndex] = { ...RUNTIME_RECORDS[recordIndex], ...updates };
            return true;
        }
        return false;
    }
